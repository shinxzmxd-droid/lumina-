...
function Page() {
  ...
  const load = () => supabase.from("leaves").select("*").eq("faculty_id", user!.id).order("created_at",{ascending:false}).then(({data})=>setRows(data ?? []));
  useEffect(() => { if (user) load(); }, [user]);

  const submit = async () => {
    const { error } = await supabase.from("leaves").insert({ faculty_id: user!.id, start_date: start, end_date: end, reason });
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold font-display">My leaves</h1>
      <Card className="p-5">
      ...
      <Card className="p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length===0 && <tr><td colSpan={3} className="p-6 text-center text-muted-foreground">No leave requests.</td></tr>}
            {rows.map(r => (
